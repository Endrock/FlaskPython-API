...
async function sectionHandle(handle, variantId) {
  const htmlResponse = await api.shopifyVariantByUrl(
    `/products/${handle}`,
    variantId,
  );
  ...
  const rewardsPoints = $Qll(
    '.points-message__text',
    stringToHTML(htmlResponse),
  );
...
/**
 * Injectnew rewards points node to the section
 *
 * @param {HTMLCollection} variantPrice - Object with the price value
 * @param {HTMLElement}
 * parent - Parent node to closest with className 'product-js'
 *
 */
function updateRewardsPoints(rewardsPoints) {
  const sectionPoints = $Qll('.points-message__text');

  sectionPoints.forEach((element, i) => {
    sectionPoints[i].innerHTML = rewardsPoints[i].innerHTML;
  });
}
...
export async function queryVariants({ target }) {
  ...
  const { value, dataset } = $Q('[name="id"]', target.closest('.product-js'));

  addcartBtn.disabled = true;
  addcartBtn.innerHTML = '<div id="loading"></div>';

  const { price, available, button, rewardsPoints } = await sectionHandle(
    dataset.variant,
    value,
  );
  ...
  if (rewardsPoints != null) {
    updateRewardsPoints(rewardsPoints);
  }
}
